export const generateEnhancedEmailContent = async (task, staffUser, adminUser) => {
  ...
  const subject = `${priorityEmoji} ${getUrgencyLevel()}: ${task.text.substring(0, 60)}${task.text.length > 60 ? '...' : ''}`;
...
${getDaysUntilDue() <= 2 ? '\n⚠️ TIME-SENSITIVE: This task requires immediate attention. Please prioritize it in your current workload.\n' : ''}
